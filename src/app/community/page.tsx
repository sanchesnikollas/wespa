...
const content = {
  en: {
    ...
    blogCards: [
      { title: 'Member Spotlight', description: '' },
      { title: 'Industry Trends', description: '' },
      { title: 'WESPA News', description: '' },
    ],
            ...
            <div className="grid gap-6 md:grid-cols-3 mb-8">
              {c.blogCards.map((card) => (
                <div key={card.title} className="p-6 bg-white border border-stone-200 rounded-2xl">
                  ...
                  {card.description && <p className="text-stone-600">{card.description}</p>}
                </div>
              ))}
            </div>
